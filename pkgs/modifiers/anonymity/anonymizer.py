...
class Anonymizer(Modifier):
    ...
    def _transform(self, text: str) -> str:
        pii_locations = sorted(
            self._find_pii_locations(text), key=lambda location: location[0]
        )
        pii_to_token = {}
        cursor = i = 0
        anonymized_string = ""
        ...
        while i < num_pii:
            ...
            while (
                i < num_pii - 1
                and end > pii_locations[i + 1][0] - 2
                and pii_type == pii_locations[i + 1][2]
            ):
                i += 1
...
class Deanonymizer(Modifier):
    ...
    def _transform(self, text: str) -> str:
        ...
        while True:
            ...
            raw_text += text[cursor:next_token_start] + self._decrypt(
                text[next_token_start + 1 : next_token_end]
            )
